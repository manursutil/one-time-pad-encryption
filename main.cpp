...
EncryptedType encrypt(const std::string &file_path,
                      const std::string &key_output_path) {
...
std::string decrypt(const std::string &key_file_path,
  ...
  while (key_file >> key_num) {
    if (key_num < 0 || key_num > 255) {
      throw std::out_of_range("Key value out of byte range: " +
  ...
  while (encrypted_file >> encrypted_num) {
    if (encrypted_num < 0 || encrypted_num > 255) {
      throw std::out_of_range("Encrypted value out of byte range: " +
                              std::to_string(encrypted_num));
    }
  ...
  if (key.size() != encrypted.size()) {
    std::cerr << "Warning: Key size (" << key.size() << ") and encrypted size ("
              << encrypted.size() << ") do not match. Using minimum size.\n";
  }
...
int main(int argc, char *argv[]) {
  try {
    if (argc < 2) {
      ...
      std::cout << "    - Encrypts <input_text_file> and outputs 'output.txt' "
                   "and 'secret_key.txt'\n";
      std::cout << "  " << argv[0]
                << " decrypt <secret_key.txt> <output.txt>\n";
      std::cout << "    - Decrypts <output.txt> using <secret_key.txt> and "
                   "prints the result\n";
